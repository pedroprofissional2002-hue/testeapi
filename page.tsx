...
export default function Home() {
  ...
  async function testar() {
    ...
    try {
      ...
      setStatus("ok"); setMsg(JSON.stringify(data, null, 2));
    } catch (e: any) { setStatus("erro"); setMsg(e.message); }
  }
  return (
    <main style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"monospace"}}>
      <div style={{width:"100%",maxWidth:"480px",padding:"2rem"}}>
        ...
        {status==="ok" && <div style={{marginTop:"1.5rem",background:"#0d1a0d",border:"1px solid #D9FF3F",borderRadius:"8px",padding:"1rem"}}><p style={{color:"#D9FF3F",marginBottom:"0.5rem"}}>✅ Sucesso!</p><pre style={{color:"#aaa",fontSize:"0.75rem",margin:0,overflow:"auto"}}>{msg}</pre></div>}
        {status==="erro" && <div style={{marginTop:"1.5rem",background:"#1a0d0d",border:"1px solid #ff4444",borderRadius:"8px",padding:"1rem"}}><p style={{color:"#ff6666",marginBottom:"0.25rem"}}>❌ Erro</p><p style={{color:"#ff9999",fontSize:"0.85rem",margin:0}}>{msg}</p></div>}
      </div>
    </main>
  );
}
